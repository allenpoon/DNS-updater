	...
	for(let i=0; i<conf.length; i++){
		let getZone = {
			...
			headers: {
				...
				'X-Auth-Email': conf[i].email,
				'X-Auth-Key': conf[i].apikey
			}
		}
		isReady++;
		https.request(getZone, (function(i){ return function(res){
			let dataChunk_GetZone="";
			res.on('data', function(data){dataChunk_GetZone += data;});
			res.on('end', function (){
				let result = [];
				try{
					result = JSON.parse(dataChunk_GetZone).result;
				}catch(e){
...
// TODO: check result.result_info.total_page
						https.request(getDNS, function(res){
							let dataChunk_GetDNS = "";
							res.on('data', function(data){dataChunk_GetDNS+=data});
							res.on('end', function(data){
								let result = [];
								try{
									result = JSON.parse(dataChunk_GetDNS).result;
								}catch(e){
